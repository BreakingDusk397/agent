...
local_path = r'/home/tristan/Desktop/agent'

x1 = glob.glob(local_path + "/*.csv")

print(x1)

stock = [pd.read_csv(i) for i in x1]

print(stock)

stocks = pd.concat(stock)
...
real_trend = stocks['Close'].tolist()
parameters = [stocks['Close'].tolist(), stocks['Volume'].tolist()]
inventory_size = 1
mean_inventory = 0.5
capital = 2
...
print(input_size)
minmax = MinMaxScaler(feature_range = (0, 1)).fit(np.array(parameters).T)
